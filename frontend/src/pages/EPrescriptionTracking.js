...
import './EPrescriptionTracking.css';

const EPrescriptionTracking = () => {
  const { user } = useAuth();
  ...
  const fetchPrescriptions = async () => {
    try {
      ...
      const sentPrescriptions = response.data.data.filter(p => 
        p.status !== 'draft' && p.transmissionInfo
      );
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'received': return '#f39c12';
      case 'processing': return '#e67e22';
      case 'ready': return '#27ae60';
      case 'dispensed': return '#2ecc71';
                ...
                >
                  ...
                  <div className="prescription-progress">
                    ...
                    <div className="progress-text">
                      {prescription.status === 'dispensed' ? 'Complete' : 
                       prescription.status === 'ready' ? 'Ready for pickup' :
                       prescription.status === 'processing' ? 'Being processed' :
                       prescription.status === 'received' ? 'Received by pharmacy' :
                       'In transit'}
                    </div>
                  </div>
                  ...
                  <div className="prescription-details">
                    ...
                    {prescription.transmissionInfo?.pharmacyConfirmation?.estimatedReadyTime && (
                      <div className="estimated-time">
                        <strong>Est. Ready:</strong> {formatDateTime(prescription.transmissionInfo.pharmacyConfirmation.estimatedReadyTime)}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="details-panel">
              <div className="details-header">
                ...
                <div className="prescription-id">
                  Rx #{selectedPrescription.prescriptionNumber}
                </div>
              </div>
                ...
                <div className="tracking-content-details">
                  ...
                  <div className="current-status">
                    <div className="status-icon">
                      {getStatusIcon(trackingDetails.currentStatus)}
                    </div>
...
export default EPrescriptionTracking;