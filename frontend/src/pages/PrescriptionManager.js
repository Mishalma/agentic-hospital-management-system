...
const PrescriptionManager = () => {
  ...
  const [selectedPharmacies, setSelectedPharmacies] = useState([
    { id: 'PHARM001', name: 'Central Pharmacy', address: '123 Main St', phone: '(555) 123-4567', distance: '0.5 miles' },
    { id: 'PHARM002', name: 'HealthMart Pharmacy', address: '456 Oak Ave', phone: '(555) 234-5678', distance: '1.2 miles' },
    { id: 'PHARM003', name: 'Community Drug Store', address: '789 Pine St', phone: '(555) 345-6789', distance: '2.1 miles' }
  ]);
  ...
  const getPrescriptionSuggestions = async () => {
    ...
    try {
      ...
      const response = await axios.post(`/api/consultations/${consultationId || 'temp'}/prescription-suggestions`, requestData);
  ...
  const getFallbackSuggestions = (diagnosis) => {
    ...
    const key = Object.keys(commonTreatments).find(condition => 
      diagnosis.toLowerCase().includes(condition)
    );

    return key ? commonTreatments[key] : {
      ...
      <div className="manager-header">
        ...
        <div className="header-actions">
          <button 
            type="button" 
            className="btn-ai" 
            onClick={getPrescriptionSuggestions}
            disabled={loadingAI}
          >
                ...
                <div className="suggestion-section">
                  ...
                  <ul>
                    {Array.isArray(aiSuggestions.medications) 
                      ? aiSuggestions.medications.map((med, index) => (
                          <li key={index}>{med}</li>
                        ))
            ...
            {drugInteractions.map((interaction, index) => (
              <div key={index} className="interaction-item">
                ...
                <div className="interaction-severity">
                  Severity: <span className={`severity-${interaction.severity}`}>
