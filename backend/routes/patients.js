...
router.get('/:patientId/basic', async (req, res) => {
  try {
    ...
    try {
      // Try findByUniqueId first (works for both MongoDB and Mock)
      patient = await Patient.findByUniqueId(req.params.patientId);
      console.log(`[PATIENT BASIC] findByUniqueId result:`, patient ? `Found ${patient.name}` : 'Not found');
    } catch (error) {
      console.log(`[PATIENT BASIC] findByUniqueId failed, trying alternative:`, error.message);
      // Fallback for MongoDB models
      try {
        patient = await Patient.findOne({ uniqueId: req.params.patientId });
        console.log(`[PATIENT BASIC] findOne result:`, patient ? `Found ${patient.name}` : 'Not found');
      } catch (altError) {
        console.log(`[PATIENT BASIC] All lookup methods failed:`, altError.message);
      }
    }
...
router.get('/:patientId/history', async (req, res) => {
  ...
  try {
    ...
    const triageRecords = await Triage.find({ patientId })
      .sort({ createdAt: -1 })
      .limit(5);
