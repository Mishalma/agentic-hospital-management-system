...
router.post('/:id?/ai-suggestions', async (req, res) => {
  try {
    ...
    const consultationData = consultation
      ? {
          symptoms: consultation.symptoms?.map(s => `${s.symptom} (severity: ${s.severity}/10, duration: ${s.duration})`).join(', '),
...
Response:`;
    ...
    if (aiResponse.success) {
      ...
      res.json({
        success: true,
        response: chatResponse || 'I understand your question. Could you provide more specific details about the clinical scenario?',
